...
public class PepelandUtilsKeyBindings {
    private static KeyMapping createKey(String id, int default_key, KeyMapping.Category category) {
        return KeyBindingHelper.registerKeyBinding(new KeyMapping("key." + MOD_ID + "." + id, InputConstants.Type.KEYSYM, default_key, category));
    }
    private static KeyMapping createKey(String id, KeyMapping.Category category) {
        return createKey(id, -1, category);
    }

    @Contract("_ -> new")
    private static KeyMapping.@NotNull Category createCategory(String name) {
        return KeyMapping.Category.register(ResourceLocation.fromNamespaceAndPath(MOD_ID, name));
    }

    public static final KeyMapping.Category CATEGORY_GENERAL = createCategory("general");

    public static final KeyMapping SEND_TO_GLOBAL_CHAT_KEY = createKey("send_to_global_chat", KeyMapping.Category.MULTIPLAYER);
